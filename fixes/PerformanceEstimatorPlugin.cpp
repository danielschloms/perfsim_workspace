...
PerformanceEstimatorPlugin::PerformanceEstimatorPlugin(
  ...
  if (monitorHandle < 0) {
    std::cout
        << "ERROR: <" << uArchName
        << "> does not name a valid variant provided by SwEvalMonitors::Factory"
        << std::endl;
  } else {
    monitor_ptr = monitorFactory.getMonitor(monitorHandle);
    if (monitor_ptr == nullptr) {
      std::cout
          << "ERROR: SwEvalMonitors::Factory failed to provide monitor for <"
          << uArchName << ">" << std::endl;
    }
  }
  ...
  if (backendHandle < 0) {
    std::cout
        << "ERROR: <" << uArchName
        << "> does not name a valid variant provided by SwEvalBackends::Factory"
        << std::endl;
  } else {
    channel_ptr = backendFactory.getChannel(backendHandle);
    if (channel_ptr == nullptr) {
      std::cout
          << "ERROR: SwEvalBackends::Factory failed to provide channel for <"
          << uArchName << ">" << std::endl;
    }
    estimator_ptr = backendFactory.getPerformanceEstimator(backendHandle);
    if (estimator_ptr == nullptr) {
      std::cout << "ERROR: SwEvalBackends::Factory failed to provide "
  ...
  if (printActive) {
    ...
    estimator_ptr->activateStreamToFile(estimateFileName.str(), outDir, ".csv",
                                        maxFileSize);
    tracePrinter_ptr->activateStreamToFile(traceFileName.str(), outDir, ".csv",
                                           maxFileSize);
...
void *PerformanceEstimatorPlugin::getPluginHandle() { return this; }
